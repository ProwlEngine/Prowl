...
public abstract class OldPropertyDrawer
{
    private static readonly Dictionary<Type, OldPropertyDrawer> _propertyDrawerLookup = new();
    ...
    public static void GenerateLookUp()
    {
        ...
        {
            List<Type> derivedTypes = EditorUtils.GetDerivedTypes(typeof(OldPropertyDrawer<>), editorAssembly);
            foreach (Type type in derivedTypes)
            {
                ...
                try
                {
                    OldPropertyDrawer propertyDrawer = Activator.CreateInstance(type) as OldPropertyDrawer ?? throw new NullReferenceException();
    ...
    protected static bool Draw(string label, ref object value, Type objType, float width = -1)
    {
        ...
        bool changed = false;
        ImGui.PushID(label);
        OldPropertyDrawer? propertyDrawer;
        ...
        else
        {
            ...
            bool found = false;
            foreach (KeyValuePair<Type, OldPropertyDrawer> pair in _propertyDrawerLookup)
...
public abstract class OldPropertyDrawer<T> : OldPropertyDrawer
{
