...
public static class EditorWallpaper
{
    ...
    public static Texture2D? Get(string path)
    {
        ...
        try
        {
            using Aperture.Image image = Aperture.Image.Load(path, new DecodeOptions { TargetPixelFormat = Prowl.Aperture.PixelFormat.Rgba8 });
            var texture = new Texture2D((uint)image.Width, (uint)image.Height, false, Prowl.Graphite.PixelFormat.R8_G8_B8_A8_UNorm);
            texture.SetData<byte>(image.RootFrame.Pixels.ToArray());
            texture.SetTextureFilters(SamplerFilter.MinLinear_MagLinear_MipLinear);
            _texture = texture;
        }
        catch (Exception ex)
        {
