...
public class HeadlessGraphicsTests
{
    ...
    [Theory]
    [InlineData(DefaultShader.Standard, FaceCullMode.Back)]
    [InlineData(DefaultShader.StandardDoubleSided, FaceCullMode.None)]
    [InlineData(DefaultShader.StandardCutoutDoubleSided, FaceCullMode.None)]
    [InlineData(DefaultShader.StandardTransparentDoubleSided, FaceCullMode.None)]
    [InlineData(DefaultShader.UnlitDoubleSided, FaceCullMode.None)]
    public void DoubleSidedShaders_DisableCulling(DefaultShader shader, FaceCullMode expected)
    {
        var loaded = Shader.LoadDefault(shader);
        foreach (var pass in loaded.Passes)
            Assert.Equal(expected, pass.State.CullMode);
    }
    ...
    public void TransparentShaders_BlendAndSkipDepthWrite(DefaultShader shader)
    {
        ...
        Assert.True(pass.State.EnableBlend);
        Assert.False(pass.State.DepthWriteMask);
        Assert.True(Shader.PassHasTag(pass, "RenderOrder", "Transparent"));
    }
