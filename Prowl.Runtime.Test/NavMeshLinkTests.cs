...
public class NavMeshLinkTests : RuntimeTestBase
{
    ...
    public void Agent_StoppedBesideALink_NeverEntersIt()
    {
        ...
        Assert.True(Float3.Distance(agent.Transform.Position, parked) < 1e-3,
            $"drifted {Float3.Distance(agent.Transform.Position, parked):0.0000} while stopped");
        ...
        {
            Tick(scene, 1);
            crossed = agent.IsOnOffMeshLink;
        }

        Assert.True(crossed, "and must cross once resumed");
    }
