...
public static class TextureUtility
{
    ...
    public static bool IsSupportedFormat(PixelFormat format, TextureType type, TextureUsage usage)
    {
        return Graphics.Device.GetPixelFormatSupport(format, type, usage);
    }
    ...
    public static PixelFormat GetBestSupportedDepthFormat(bool needsStencil = false, TextureUsage usage = TextureUsage.DepthStencil)
    {
        ...
        if (Graphics.Device.GetPixelFormatSupport(PixelFormat.D16_UNorm_S8_UInt, TextureType.Texture2D, usage))
            return PixelFormat.D16_UNorm_S8_UInt;

        if (!needsStencil)
        {
            if (Graphics.Device.GetPixelFormatSupport(PixelFormat.D16_UNorm, TextureType.Texture2D, usage))
                return PixelFormat.D16_UNorm;
        }

        throw new Exception("No valid supported depth formats");
    }
}
