...
public static class Graphics
{
    ...
    // Texture read-backs recorded onto a frame that was already open when requested (see
    // Texture.ReadBackSubresource) can't be mapped until that frame's GPU work actually finishes.
    // Each entry is drained once its frame completes; must only be checked while no frame is open,
    // since IsFrameComplete on the currently-open frame's id would still be false anyway.
    private readonly struct PendingReadback(Texture staging, ulong frameId, Action<MappedResource> consume, Action onComplete)
    {
        public readonly Texture Staging = staging;
        public readonly ulong FrameId = frameId;
        public readonly Action<MappedResource> Consume = consume;
        public readonly Action OnComplete = onComplete;
    }

    private static readonly List<PendingReadback> s_pendingReadbacks = new();

    /// <summary>Queue a texture read-back whose copy was recorded onto an already-open frame; it will
    /// be mapped and consumed once that frame completes on the GPU.</summary>
    internal static void QueueReadback(Texture staging, ulong frameId, Action<MappedResource> consume, Action onComplete)
    {
        lock (s_pendingReadbacks)
            s_pendingReadbacks.Add(new PendingReadback(staging, frameId, consume, onComplete));
    }

    /// <summary>Maps, consumes, and disposes any queued read-back whose frame has completed on the
    /// GPU. Called once per tick by the frame loop while no frame is open.</summary>
    public static void FlushPendingReadbacks()
    {
        GraphicsDevice device = Device;
        if (device == null) return;
        ...
        lock (s_pendingReadbacks)
        {
            for (int i = s_pendingReadbacks.Count - 1; i >= 0; i--)
            {
                PendingReadback pending = s_pendingReadbacks[i];
                if (!device.IsFrameComplete(pending.FrameId))
                    continue;

                MappedResource mapped = device.Map(pending.Staging, MapMode.Read);
                try { pending.Consume(mapped); }
                finally { device.Unmap(pending.Staging); }
                pending.Staging.Dispose();
                pending.OnComplete?.Invoke();

                s_pendingReadbacks.RemoveAt(i);
            }
        }
    }
}
