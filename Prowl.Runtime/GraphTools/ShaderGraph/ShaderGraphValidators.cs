...
/// <summary>Flag a graph that's missing — or has more than one of — the master output node.</summary>
...
public sealed class MasterOutputValidator : GraphValidator
{
    public override void Validate(Graph graph)
    {
        ...
        foreach (var n in graph.Nodes)
            if (n is Nodes.PBROutputNode) { count++; if (first == null) first = n; }

        if (count == 0)
        {
            // Surface as an info message on the first node (or in the log if the graph
            // is totally empty — handled by the editor's diagnostics route).
            if (graph.Nodes.Count > 0)
                graph.Nodes[0].Messages.Add(new NodeMessage {
                    Severity = NodeMessageSeverity.Error,
                    Text = "Shader graph has no Output node — add a PBR Output node.",
                });
        }
        else if (count > 1)
        {
            foreach (var n in graph.Nodes)
                if (n is Nodes.PBROutputNode)
