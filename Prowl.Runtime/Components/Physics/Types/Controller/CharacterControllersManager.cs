...
public unsafe class CharacterControllersManager : IDisposable
{
    readonly BufferPool pool;

    Buffer<int> bodyHandleToCharacterIndex;
    QuickList<CharacterControllerData> characters;
    ...
    public CharacterControllersManager(BufferPool pool, int initialCharacterCapacity = 4096, int initialBodyHandleCapacity = 4096)
    {
        this.pool = pool;
    ...
    bool TryReportContacts<TManifold>(CollidableReference characterCollidable, CollidableReference supportCollidable, CollidablePair pair, ref TManifold manifold, int workerIndex) where TManifold : struct, IContactManifold<TManifold>
    {
        if (characterCollidable.Mobility == CollidableMobility.Dynamic && characterCollidable.BodyHandle.Value < bodyHandleToCharacterIndex.Length)
        {
            ...
            if (characterIndex >= 0)
            {
                ...
                ref var set = ref Physics.Sim.Bodies.Sets[bodyLocation.SetIndex];
                ...
                if (manifold.Convex)
                {
                    ...
                    var normalUpDot = System.Numerics.Vector3.Dot(convexManifold.Normal, up);
                ...
                else
                {
                    ...
                    for (int i = 0; i < nonconvexManifold.Count; ++i)
                    {
                        ...
                        if (candidate.Depth > maximumDepth)
                        {
                            ...
                            var upDot = System.Numerics.Vector3.Dot(candidate.Normal, up);
    ...
    unsafe void ExpandBoundingBoxes(int start, int count)
    {
        ...
        for (int i = start; i < end; ++i)
        {
            ...
            var characterBody = Physics.Sim.Bodies[character.BodyHandle];
            if (characterBody.Awake)
            {
                Physics.Sim.BroadPhase.GetActiveBoundsPointers(characterBody.Collidable.BroadPhaseIndex, out var min, out var max);
                ...
                *min += System.Numerics.Vector3.Min(System.Numerics.Vector3.Zero, supportExpansion);
                *max += System.Numerics.Vector3.Max(System.Numerics.Vector3.Zero, supportExpansion);
            }
        }
    }
    ...
    void AnalyzeContactsForCharacterRegion(int start, int exclusiveEnd, int workerIndex)
    {
        ...
        for (int characterIndex = start; characterIndex < exclusiveEnd; ++characterIndex)
        {
            ...
            if (bodyLocation.SetIndex == 0)
            {
                ...
                if (character.Supported)
                {
                    ...
                    if (!Physics.Sim.Solver.ConstraintExists(character.MotionConstraintHandle) ||
                        ...
                        (Physics.Sim.Solver.HandleToConstraint[character.MotionConstraintHandle.Value].TypeId != DynamicCharacterMotionTypeProcessor.BatchTypeId &&
                         Physics.Sim.Solver.HandleToConstraint[character.MotionConstraintHandle.Value].TypeId != StaticCharacterMotionTypeProcessor.BatchTypeId))
                    {
                ...
                {
                    QuaternionEx.Transform(character.LocalUp, Physics.Sim.Bodies.ActiveSet.DynamicsState[bodyLocation.Index].Motion.Pose.Orientation, out var characterUp);
                    ...
                    var characterUpVelocity = System.Numerics.Vector3.Dot(Physics.Sim.Bodies.ActiveSet.DynamicsState[bodyLocation.Index].Motion.Velocity.Linear, characterUp);
                    ...
                    {
                        ref var supportingBodyLocation = ref Physics.Sim.Bodies.HandleToLocation[character.Support.BodyHandle.Value];
                        Debug.Assert(supportingBodyLocation.SetIndex == 0, "If the character is active, any support should be too.");
                        ref var supportVelocity = ref Physics.Sim.Bodies.ActiveSet.DynamicsState[supportingBodyLocation.Index].Motion.Velocity;
                        var wxr = System.Numerics.Vector3.Cross(supportVelocity.Angular, supportCandidate.OffsetFromSupport);
                        var supportContactVelocity = supportVelocity.Linear + wxr;
                        var supportUpVelocity = System.Numerics.Vector3.Dot(supportContactVelocity, characterUp);
                ...
                else if (supportCandidate.Depth > float.MinValue)
                {
                    ...
                    QuaternionEx.Transform(character.LocalUp, Physics.Sim.Bodies.ActiveSet.DynamicsState[bodyLocation.Index].Motion.Pose.Orientation, out var up);
                    var rayDistance = System.Numerics.Vector3.Dot(character.ViewDirection, surfaceBasis.Y);
                    var rayVelocity = System.Numerics.Vector3.Dot(up, surfaceBasis.Y);
                    ...
                    {
                        QuaternionEx.GetQuaternionBetweenNormalizedVectors(System.Numerics.Vector3.UnitY, surfaceBasis.Y, out var rotation);
                        QuaternionEx.TransformUnitZ(rotation, out surfaceBasis.Z);
                    }
                    surfaceBasis.X = System.Numerics.Vector3.Cross(surfaceBasis.Y, surfaceBasis.Z);
                    ...
                    if (supportCandidate.Support.Mobility != CollidableMobility.Static)
                    {
                        ...
                        if (character.Supported && !shouldRemove)
                        {
                            ...
                            Physics.Sim.Solver.ApplyDescriptionWithoutWaking(character.MotionConstraintHandle, motionConstraint);
                        }
                        else
                        {
    ...
    void AnalyzeContacts(float dt, IThreadDispatcher threadDispatcher)
    {
        ...
        if (analyzeContactsWorkerCaches.Allocated)
        {
            ...
            for (int threadIndex = 0; threadIndex < analyzeContactsWorkerCaches.Length; ++threadIndex)
            {
                ...
                {
                    Physics.Sim.Solver.Remove(cache.ConstraintHandlesToRemove[i]);
                }
            }
            for (int threadIndex = 0; threadIndex < analyzeContactsWorkerCaches.Length; ++threadIndex)
            {
                ...
                for (int i = 0; i < workerCache.StaticConstraintsToAdd.Count; ++i)
                {
                    ...
                    character.MotionConstraintHandle = Physics.Sim.Solver.Add(character.BodyHandle, pendingConstraint.Description);
                }
                for (int i = 0; i < workerCache.DynamicConstraintsToAdd.Count; ++i)
                {
                    ...
                    ref var character = ref characters[pendingConstraint.CharacterIndex];
                    Debug.Assert(character.Support.Mobility != CollidableMobility.Static);
                    character.MotionConstraintHandle = Physics.Sim.Solver.Add(character.BodyHandle, character.Support.BodyHandle, pendingConstraint.Description);
                }
                ref var activeSet = ref Physics.Sim.Bodies.ActiveSet;
                for (int i = 0; i < workerCache.Jumps.Count; ++i)
                {
                    ...
                    {
                        BodyReference.ApplyImpulse(Physics.Sim.Bodies.ActiveSet, jump.SupportBodyIndex, jump.CharacterVelocityChange / -activeSet.DynamicsState[jump.CharacterBodyIndex].Inertia.Local.InverseMass, jump.SupportImpulseOffset);
                    }
                }
    ...
    public void Dispose()
    {
        ...
        {
            disposed = true;
            Physics.Sim.Timestepper.BeforeCollisionDetection -= PrepareForContacts;
            Physics.Sim.Timestepper.CollisionsDetected -= AnalyzeContacts;
