...
void main()
{
	...
	vec3 direction = normalize(fragPos - Camera_WorldPosition);
	...
	vec3 pixelPosition = Camera_WorldPosition + (direction * depth);
	
	//gBuffer_lighting = vec4(pixelPosition, 1.0);
	
    //vec4 albedoSpec = texture(gAlbedo, fragCoord);
    vec3 normal = texture(texture0, fragCoord).rgb * 2.0 - 1.0;
	
	
    vec3 toLight = LightPosition - pixelPosition;
    ...
    float attenuation = max(1.0 - (dist*dist) / (LightRadius*LightRadius), 0.0);
