Shader "Default/ProceduralSkybox"
{
    Pass
    {
        ...
        float3 atmosphereNoMie(float3 r, float3 r0, float3 pSun, float iSun, float rPlanet, float rAtmos, float3 kRlh, float shRlh) {
            pSun = normalize(pSun);
            r = normalize(r);
            ...
            for (int i = 0; i < iSteps; i++) {
                ...
                float3 attn = exp(-(kRlh * (iOdRlh + jOdRlh)));
                totalRlh += odStepRlh * attn;
                iTime += iStepSize;
            }
        ...
        {
            // Remove translation from view matrix (GLSL V[3][i] -> Slang V[i][3]).
            ...
            Varyings o;
            o.position = mul(mul(Frame.prowl_MatP, viewNoTranslation), float4(input.position, 1.0));
            o.position.z = o.position.w;

            o.vDirection = input.position;
            o.vSunDir = normalize(Mat._SunDir);
            o.vSkyColor = atmosphere(
                normalize(o.vDirection),          // normalized ray direction
                float3(0,6372e3,0),               // ray origin
                o.vSunDir,                        // position of the sun
